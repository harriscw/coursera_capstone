...
shinyServer(function(input, output) {
  ...
  myfunc <- function(myentry,model="WEIGHTED",oneword=""){
    ...
    if (!(toupper(model) %in% c("EQUAL","WEIGHTED","BACK"))){return("Model weight not valid.  Enter either 'equal', 'weighted', or 'back' ")}else{
        ...
        if(nrow(d)>0){
          ...
          if(toupper(oneword) == "YES"){
            returndf=as.character(returndf[1,1])
          }
          ...
          labelmax=max(nchar(mytable1$suggestion))
          par(mar=c(5, labelmax+1, 4, 2) + 0.1)
  ...
  output$value3 <- renderPrint({ input$action })
  
  

  mypreds <- eventReactive(input$action, {
    input$text
  })

  output$value4 <- renderPrint({ myfunc(mypreds(),model=input$radio)$a })
  
  output$value5 <- renderPlot({ myfunc(mypreds(),model=input$radio)$b })
